...
# read data ---------------------------------------------------------------
  ...
  dat <- read_csv(here::here("theory/result/result_sim2020-11-10.csv")) %>% 
    ...
    filter(metric == "gamma_div") %>% 
    mutate(group = case_when(between(n_patch, 40, 60) ~ "Small ecosystem",
                             between(n_patch, 90, 110) ~ "Medium ecosystem",
                             n_patch > 130 ~ "Large ecosystem")
    )
...
# complexity effect -------------------------------------------------------
  ...
  g <- dat %>% 
    ggplot(aes(x = p_branch, y = value, color = group, fill = group)) +
