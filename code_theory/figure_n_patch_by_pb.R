...
# read data ---------------------------------------------------------------
  ...
  dat <- read_csv(here::here("theory/result/result_sim2020-11-10.csv")) %>% 
    ...
    mutate(group = case_when(p_branch < 0.3 ~ "Low complexity",
                             between(p_branch, 0.3, 0.6) ~ "Medium complexity",
                             p_branch > 0.6 ~ "High complexity")
           )
...
# ecosystem size effect ---------------------------------------------------
  ...
  g <- dat %>% 
    ggplot(aes(x = n_patch, y = value, color = group, fill = group)) +
