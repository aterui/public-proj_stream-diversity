...
# Hokkaido data
dat_hkd <- read_csv(here::here('/empirical/data_org_hkd/data_fmt_hkd_latest.csv')) %>% 
  filter(Year >= 1990)

# Midwest data
fishdata <- list.files(path = here::here("/empirical/data_org_mw"), full.names = TRUE) %>% lapply(read_csv)
dat_mw <- do.call(what = bind_rows, args = fishdata)
...
1. We listed recorded fish species and re-organized species names to be consistent across the data sets.
...
1. Four genera (Lethenteron, Pungitius, Rhinogobius, and Tribolodon) were treated as species groups (i.e., spp.) as taxonomic resolutions varied greatly among data sources.
...
## Table S1 Species list in Hokkaido, Japan

Species list in Hokkaido, Japan. Species are ordered alphabetically.

```{r chunk_table1}

dat_hkd %>% 
  filter(Year >= 1990) %>% 
  mutate(Species = str_replace_all(str_to_title(OTU), "_", " ")) %>% 
