...
# setup -------------------------------------------------------------------
  ...
  pacman::p_load(tidyverse, MuMIn, gt)
...
# read data ---------------------------------------------------------------
  ...
  ## gamma
  fit <- lm(log(gamma) ~ log(area)*region + log(p_branch)*region + scale(mean_temp) + scale(mean_ppt) + scale(logit_forest), dat)
  m <- dredge(fit, rank = "AIC")
  re <- model.avg(object = m, subset = delta < 2)
  x <- summary(re)
  knitr::kable(x$coefmat.subset)
  predict(re$call)
```
